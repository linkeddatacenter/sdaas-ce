...
@test "Ingestion platform acceptance test..." {
	...
	[[ "${lines[1]}"  =~ "SD_START_LOGGING debug info in /tmp/sdaas/session_"  ]]
	...
	[[ "${lines[10]}" =~ "downloaded https://schema.org/version/latest/schemaorg-current-https.ttl"  ]]
	...
	[[ "${lines[27]}" =~ "2_empty.select...OK"  ]]
	[[ "${lines[28]}" =~ "Knowledge ingestion succesfully completed"  ]]
	[[ "${lines[29]}" =~ "graphName,RDF_statements"  ]]
	[[ "${lines[30]}" =~ "urn:graph:istat,15981"  ]]
	[[ "${lines[31]}" =~ "http://schema.org/"  ]]
	[[ "${lines[32]}" =~ "urn:kees:config,55"  ]]
	[[ "${lines[33]}" =~ "urn:graph:calculated_trusts,23"  ]]
	[[ "${lines[34]}" =~ "urn:graph:trustmap_default,13"  ]]
}
