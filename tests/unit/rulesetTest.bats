...
@test "SD_EVAL_RULESET" {
	...
	#echo "$output" > /tmp/x
	[[ "${lines[0]}" =~ 'starded calling axiom 1_test from  ruleset 01_ruleset' ]]
	...
	[[ "${lines[7]}" =~ 'SD_SPARQL_UPDATE @-' ]]
	[[ "${lines[8]}" =~ 'starded calling axiom 3_test from  ruleset 01_ruleset' ]]
	[[ "${lines[9]}" =~ 'SD_SPARQL_UPDATE DROP SILENT GRAPH <urn:graph:tests/unit/data/01_ruleset/3_test.reasoning>' ]]
	[[ "${lines[10]}" =~ 'SD_LOAD_RDF_FILE urn:graph:tests/unit/data/01_ruleset/3_test.reasoning' ]]
}
